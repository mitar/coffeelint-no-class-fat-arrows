module.exports = class NoClassFatArrows
  ...
  isCode: (node) ->
    @astApi.getNodeName(node) is 'Code'

  isClass: (node) ->
    @astApi.getNodeName(node) is 'Class'

  isValue: (node) ->
    @astApi.getNodeName(node) is 'Value'

  isObject: (node) ->
    @astApi.getNodeName(node) is 'Obj'

  isFatArrowCode: (node) ->
    @isCode(node) and node.bound
  ...
  methodsOfClass: (classNode) ->
    ...
    if returnNode? and @isValue(returnNode) and @isObject(returnNode.base)
      returnNode.base.properties.map((assignNode) => assignNode.value).filter((node) => @isCode node)
    else
      []
